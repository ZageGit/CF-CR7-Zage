...
// Function for getting items array and creating sum and displaying//
  ...
  ngDoCheck() {
    ...
    this.items.forEach(function(tour){sum = sum+tour.price}); // loop through array and adding price parameter//
    var paySum=0; // creating paySum variable, used for if /else statement to calculate discount//
    if (sum<200){ // if paySum lower 200, 0% discount, displaying paySum//
      var paySum=sum;
      document.getElementById("total").innerHTML = "Total: "+ paySum + "€";

    }
    else if(sum<=499) // else if paySum lower 499, 10% discount, displaying paySum and discount//
    {var paySum= sum*0.9;
      document.getElementById("total").innerHTML = "Total: "+ paySum + "€";
      document.getElementById("discount").innerHTML = "Discount: -10%";

    }
    else if (sum>499){ // else if paySum greater 499, 20% discount, displaying paySum and discount//
      var paySum = sum*0.8;
      document.getElementById("total").innerHTML = "Total: "+ paySum + "€";
  ...
  onSubmit(customerData) { // on order Submit console warn, window alert, clearing array and clearing content of discount//
